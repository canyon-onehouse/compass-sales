...
- **Lead capture is first-party.** Our forms post to a site-owned endpoint; the site keeps no lead store *(refined by ADR 0003: a transient 90-day delivery buffer guards against CRM outages — the CRM stays the sole system of record)*; a Plan request always carries its Plan's slug. What sits behind the endpoint (GoHighLevel directly, an interim relay such as Formspree) is a swappable delivery detail.
